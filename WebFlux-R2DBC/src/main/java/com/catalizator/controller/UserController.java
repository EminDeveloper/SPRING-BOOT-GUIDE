package com.catalizator.controller;

import com.catalizator.config.JwtUtil;
import com.catalizator.domain.User;
import com.catalizator.service.UserService;
    ...
    public Mono<ResponseEntity> login(ServerWebExchange swe){
        return swe.getFormData().flatMap(credentials ->
                userService.findByUsername(credentials.getFirst("username"))
                        ...
                        .map(userDetails ->
                                Objects.equals(
                                        credentials.getFirst("password"), userDetails.getPassword()
                                )
